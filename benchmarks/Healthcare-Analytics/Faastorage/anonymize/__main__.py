...
def main(event):
    ...
    try:
        ...
        bucket_get = df.get_bucket("kingdo-identifyphi")
        message = bucket_get.get("message")
        entity = bucket_get.get_bytes("entities")
        ...
        event = timestamp({}, event, 0, 0, transport_start_time, transport_end_time)

        transport_start_time = endTime
        bucket = df.create_bucket("kingdo-anonymize", 1024 * 4)
        bucket.set("message", masked_message)
        transport_end_time = 1000 * time.time()
