...
int df::utils::hardware_concurrency() { return (int)std::thread::hardware_concurrency(); }
    ...
    std::string getPrimaryIPForThisHost(const std::string& interface)
    {
        ...
        for (ifa = allAddrs; ifa != nullptr; ifa = ifa->ifa_next)
        {
            ...
            if (interface.empty())
            {
                ...
                if (df::utils::startsWith(ifaceName, "eth") || df::utils::startsWith(ifaceName, "wl") || df::utils::startsWith(ifaceName, "en"))
                {
