...
namespace df::dataStruct::KV_Store {
    ...
    public:
        ...
        static std::shared_ptr<StateFunctionKVStoreBucket> CreateBucket(const std::string &name, size_t size) {
            return std::make_shared<StateFunctionKVStoreBucket>(name, size);
        }

        static std::shared_ptr<StateFunctionKVStoreBucket> getBucket(const std::string &name) {
            return std::make_shared<StateFunctionKVStoreBucket>(name);
        }

        StateFunctionKVStoreBucket(std::string name, size_t size) :
        ...
        explicit StateFunctionKVStoreBucket(std::string name) : bucket_name(std::move(name)), bucket_size(0) {
